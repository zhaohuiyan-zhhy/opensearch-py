...
utils/aos_api_spec/opensearch-openapi.yaml
        +
utils/aos_api_spec/overlays/amazon-serverless.overlay.yaml
        |
        v
...
AOS 和 AOSS 共用 `utils/generate_aos_api.py` 中的通用能力。`ClientConfig` 固定每种
distribution 的输入、输出、类名和 surface 验证规则。

AOSS 入口为：

```text
utils/generate_aoss_api.py
```
...
```bash
python -m utils.aoss_live_smoke_test \
